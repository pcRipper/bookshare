...
| `/library` | `LibraryView` | The signed-in user's library. Profile header (avatar, name, bio, stats) + **five** tabs: **Books** (book grid, with a **text search** + CSV **import/export** toolbar), **Collections** (owner's curated groups — create/edit/delete; see _Book collections_), **Wish List** (books wanted rather than held, with a priority sort + filter; see _Wish list_), **Sharing** (the whole loan lifecycle — see _Sharing tab_), **Following**. Collection loans surface grouped throughout Sharing. Five tabs still overflow a phone (565px of tabs in 342px at 390 wide), so the strip scrolls horizontally with **edge shadows** as the affordance (`.tab-nav`, `LibraryView.vue`) — the scrollbar is hidden, and without them the off-screen tabs read as missing |
...
| `/admin/stats` | `AdminStatsView` | **Admin only** — the operator dashboard: growth, engagement, traffic and library health, over a 7/30/90-day window. Reached from the account dropdown, which only shows the link to an administrator. See _Analytics_ |
...
- **Two subtabs — Borrowing (books you hold) / Lending (books you own)** — rendered by the `.subtab-nav` pill pair, the control the history toggle used, so this is a rename plus a badge, not a new component. `sharingSide` (`'borrowing' | 'lending'`) is the single state.
