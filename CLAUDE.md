...
Every controller under `/api/admin` is gated **twice** — the `access_control` rule is the coarse net covering any controller added later, the `#[IsGranted]` attribute supplies a message we control. `AdminAccessConfigTest` sweeps `src/Controller/Admin*RestController.php` with a data provider, so a controller added to the panel is covered the day it lands.
