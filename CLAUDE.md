...
- **Badges and counts are different things and must stay different.** A `badge` (navy) means something is waiting for you, which is why Sharing has them; a `count` (muted) is how big a shelf is, which is not a task. So the parent Books tab carries **no badge** and its three pills carry counts. Every count comes off the one eager `fetchMe()` — `UserStatsProvider` supplies `totalBooks`/`collections`/`wished` together — so none of them reads 0 until its shelf is opened, which is the trap the Sharing badges had to be written around.
