...
| Subprocesses | `symfony/process` — only caller is `PgDump` (see _Admin panel_) |
...
| `/discover` | `DiscoverView` | Browse the community in two modes — **books** (search, category filter pills, **language filter**, a **Newest / Top rated** sort, card/table view) and **readers** (`/users/discover`, follow/unfollow inline). **Neither mode needs a query**: an empty box browses — books newest-first, readers newest-member-first — and typing filters (books by title/author, readers by name, alphabetically) |
| `/profile/:id` | `ProfileView` | Public profile. Avatar, bio, stats; tabs for the **read-only** book shelves (Available to Borrow / All Books, with a **text search**), a **Collections** tab (read-only, each with a "Borrow collection" modal — see _Book collections_) and a read-only **Wish List** tab, all with "Request to Borrow" (own profile is a preview — book/collection CRUD lives in `/library`, not here; a wish-list book offers no borrow action at all). The **one** thing written here rather than in the library is the book's **review**, on its own tab in the detail modal — a verdict is not inventory (see _Ratings & reviews_) |
...
php bin/console app:dump --kind=sql          # database dump → var/dumps (--kind=json, --list)
...
- **Frontend**: `stores/collections.js`; components under `components/collections/` — `CollectionCard` (portrait 2/3 cover to match book cards, falling back to `CollectionCoverMotif` — the shared no-cover hint, a fan of three member covers on a card and a gapless mosaic filling the detail sheet's larger area, which is why it is shared: the fan lived on the card alone and the sheet fell through to a static glyph; **no on-card buttons** — the whole card opens the editor on the Library tab / the preview on a Profile, mirroring `BookCard`; an "On loan" badge marks a frozen one), `CollectionEditModal` (cover preview + URL at the top, then name/description + a **two-pane book picker** — the selected books above a **searchable** add-list — plus a Delete action and a read-only on-loan mode; mirrors `ManageBookModal`), `CollectionBorrowModal` (the collection parallel of `BookDetailModal`: a read-only **preview** — cover, owner, description, member books — that doubles as the borrow dialog for non-owners, locking non-`own`/already-`requested` rows and requiring ≥2; `isSelf` ⇒ pure preview), and — for the borrow lifecycle itself — no card of its own: a collection loan is rendered by the shared `library/LoanCard` alongside the per-book ones (see _Sharing tab_). Every collection card/request/loan is badged **"Collection"** so it's never mistaken for a single-book item.

### Ratings & reviews

A book carries the **owner's own verdict on it**: a 1-5 star rating and, beside it, up to 1000 words. Both live on `Book` (`rating`, `review`) rather than in an entity of their own — the same call the wish list makes.

- **It is one person's judgement and can never be an average.** A `Book` row is one member's *physical copy* — five members owning Dune are five rows — so a score aggregated per copy would sit at n=1 for nearly every book and read as precision the data doesn't have. In a lending app the useful signal is anyway the taste of the person you would borrow *from*. A community layer (a `BookRating` entity, earned by a **returned** loan — you may rate what you demonstrably read) can be added on top later without changing any of this; it was scoped out deliberately, not forgotten.
- **`PUT /books/{id}/review` is the only write path.** `BookInput` deliberately carries **neither** field, which is what makes a PATCH from the Manage Book modal unable to disturb a review — the trap `toBookInput` used to warn about, closed rather than doubled. Its own endpoint for the reason `POST /{id}/acquire` has one: a single deliberate act with no other input. `App\Dto\BookReviewInput` is the DTO, and **the CSV importer validates every row through the same one**, so a file and the UI are held to identical rules. Sending nulls clears a review — there is no DELETE, because "no rating, no words" is the state every book starts in.
- **Guarded on plain ownership, never `BookVoter::EDIT`.** That voter also requires the book to be **home**, and a book at a borrower's house has nothing to do with what its owner thought of it. Being unable to rate a book you had just lent out was the concrete failure of the first cut, which put the control inside the edit form — the same form that locks wholesale on `canEdit`. Lent, unavailable, currently-reading and wish-list books are all reviewable.
- **Null is meaningful** — "not rated" is not "rated badly" — so every surface `v-if`s on the value and an unrated book renders **nothing**, exactly like `book.language`. A column of empty scales down a shelf would be worse than no feature.
- **The rating is rejected, not clamped** (`Assert\Range(1, 5)`), the deliberate opposite of `wishPriority`'s normalise-don't-422 rule: a stale priority is a field the client wasn't thinking about, whereas 6 stars means it is counting something else. Both stay **orthogonal to `is_read`** — rating a book you never ticked as read is unusual, not incoherent.
- **Public**, and `ResponseMapper::publicBook()` says so on purpose: the owner's opinion of a book they are offering to lend has the same standing as the "Read" badge and names no third party. The reason to write a review is for other readers to read it.
- **Discover sorts on the rating** — `?sort=rating`, clamped to newest-first for any other value (the browse rule `Pagination` follows). Ranked on a **HIDDEN `COALESCE(b.rating, 0)`**, not on the column: DQL has no `NULLS LAST`, PostgreSQL sorts nulls *first* on a DESC column, and `COALESCE` is only grammatical in the SELECT — so without it the unrated books would lead the one list whose whole point is the rated ones.
- **Writing happens on the profile, never in the library.** `ui/BookDetailModal.vue` carries **Details / Review** tabs over its info column (on the shared `ui/ModalTabs.vue`, lifted out of `ManageBookModal` for this), so the cover stays put while the right side swaps. The editable state rides a **`canReview` prop and deliberately not `isSelf`** — `PublicLibraryView` passes `is-self` to mean "no borrow button", so gating an editor on it would hand one to a signed-out visitor; only `ProfileView` passes `canReview`, and only for the viewer's own profile. Other viewers get prose (with a byline where the payload carries an `owner`, i.e. Discover), and **the tab is not rendered at all when there is nothing behind it** — a dead tab is worse than none.
- **Display is compact in lists, expressive in detail**: `ui/StarRating.vue` is the app's only star surface — one filled star plus the number on cards and in `BookTable` (read-only everywhere, the library included), the full five-glyph row in the detail sheet and the editor. Filled stars take `--color-accent` (the brass), a set star being exactly the "active/selected" state the accent is reserved for. Clicking the current star clears it, which is the only route back to unrated.
- **CSV carries both** (`rating`, `review`); a file exported before either column imports as unreviewed, the header map being name-based.
- Deliberately **not** on the admin dashboard — no rating section, no "top rated" ranking there.
...
**Dumps** (`AdminDumpRestController` at `/admin/dumps`, `App\Service\Admin\DumpService`, `stores/adminDumps.js`, `AdminDumpsView`). Two kinds, and the UI presents them as two *things* rather than one format picker, because only one is a backup:

| Kind | What it is | Written by |
|---|---|---|
| `sql` | `pg_dump --format=custom` — compressed, restorable with `pg_restore`, selective restore included | `PgDump` |
| `json` | every table's rows, streamed one at a time | `JsonExporter` |

The JSON kind goes **table-by-table through DBAL, not entity-by-entity through the ORM**. The point of the file is to show what is actually stored, so the mapping layer would be exactly wrong: it would silently drop the join tables and the `_audit` trail (no entities behind them), re-key columns into API names, and drift with every mapping change. Raw rows need no mapper and cannot disagree with the database. It skips only `doctrine_migration_versions` (a ledger for a schema this file doesn't carry) and `messenger_messages` (transient). **`listTableNames()` returns reserved-word tables already quoted** (`"user"`), so the name is unquoted before being re-quoted — otherwise the query asks for a table literally called `"user"`, quotes included.

**Two things about `pg_dump` are easy to get wrong and both live in `PgDump`.** The password goes through `PGPASSWORD` in the process environment, **never argv**, which `ps` publishes on a shared host. And the client major version is pinned — `postgresql16-client` in both images against the `postgres:16` service — because pg_dump refuses outright to dump a server newer than itself and Alpine's unversioned package tracks whatever is current; bump the two together or not at all. Where the binary is **absent** — a dev machine running PHP outside the container, which is the normal case, not a fault — `supports()` says so, the list endpoint reports it in `capabilities`, and the panel disables the button with an explanation rather than offering an action that always 503s.

**`DumpService::path()` is the security boundary.** `$name` arrives from a URL, so anything looser than an exact pattern is a file-read primitive — `..%2f..%2f.env` is the whole attack. Two independent checks, because they fail differently: the name pattern refuses a name that could traverse, and a `realpath` prefix comparison refuses one that traverses *anyway*, through a symlink the pattern cannot see. Eleven hostile names are pinned by `DumpServiceTest`. Dumps live under **`var/`, never `public/`** — one carries every member's email and their whole library — and `/var/` is already gitignored, so none can be committed by accident.

**The filesystem is the record**: there is no `dump` table, because a second source of truth goes wrong the first time somebody removes a file over SSH. Listing scans the directory and ignores anything that isn't one of ours. Retention keeps the **newest ten per kind** (per kind, so a run of JSON exports can't push out the SQL backups), pruned on create, since nothing prunes on a schedule and an operator clicking weekly for a year must not fill the droplet.
