...
A book carries the **owner's own 1-5 star rating**, on `Book.rating` rather than in an entity of its own — the same call the wish list makes, and for the same reason: cataloguing already writes the whole `BookInput`, so a rating needs no endpoint, no service and no new write path.
...
- **Rejected, not clamped** (`BookInput`'s `Assert\Range(1, 5)`), which is the deliberate opposite of `wishPriority`'s normalise-don't-422 rule: a stale priority is a field the client wasn't thinking about, whereas 6 stars means it is counting something else, and quietly storing 5 would hide that. It is also **orthogonal to `is_read`** — rating a book you never ticked as read is unusual, not incoherent, so nothing couples them (unlike `setWish()`, where the third combination is meaningless).
- **Public**, and `ResponseMapper::publicBook()` says so on purpose: the owner's opinion of a book they are offering to lend has the same standing as the "Read" badge and names no third party.
- **Discover sorts on it** — `?sort=rating`, clamped to newest-first for any other value (the browse rule `Pagination` follows). Ranked on a **HIDDEN `COALESCE(b.rating, 0)`**, not on the column: DQL has no `NULLS LAST`, PostgreSQL sorts nulls *first* on a DESC column, and `COALESCE` is only grammatical in the SELECT — so without it the unrated books would lead the one list whose whole point is the rated ones.
- **Frontend**: `ui/StarRating.vue` is the app's only star surface, with the split **compact in lists, expressive in detail** — one filled star plus the number on cards and in `BookTable`, the full five-glyph row in `BookDetailModal`. Filled stars take `--color-accent` (the brass), a set star being exactly the "active/selected" state the accent is reserved for. **Editing lives only in the Manage Book modal**: five targets in a 32px table row is not a touch target, so the table's rating cell is read-only even in your own library (unlike the read checkbox). Editable mode is a `radiogroup` — arrow keys move it, one tab stop — and clicking the current star clears it, which is the only route back to unrated. `utils/bookPayload.js`'s `toBookInput` **must** carry `rating`, or the inline read toggle (which resends the whole DTO) wipes it.
- **CSV carries it** in the `rating` column; a file exported before it imports as unrated, the header map being name-based.
