...
- **CRUD**: `GET /api/collections?owner=&page=` (paginated; private-profile 403 guard, viewer-relative `requested` flag on each book), `GET /api/collections/{id}`, `POST/PATCH/DELETE /api/collections/{id}`. `App\Service\CollectionService` (persist-never-flush) validates ownership of the member books and blocks deletes/edits while the collection is actively borrowed. `App\Security\Voter\CollectionVoter` (`COLLECTION_EDIT`/`COLLECTION_DELETE`: owner **and** not actively borrowed) drives the emitted `canEdit`.
