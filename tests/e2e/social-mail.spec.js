...
import { BORROWER, OWNER, authHeaders, updateSettings } from './helpers/app.js'
...
test.describe('follow mails', () => {
  ...
  test('following someone who opted in mails them', async ({ request }) => {
    await updateSettings(request, OWNER, { notifyActivity: true })
    ...
    try {
      ...
      await request.delete(`/api/subscriptions/${ownerId}`, { headers: authHeaders(BORROWER) })
      await updateSettings(request, OWNER, { notifyActivity: false })
    }
  })
