...
- [x] revise borrowing-lending tabs in profile/library, lets merge them and introduce subtabs in single tab "Sharing" — Borrowing, Lending, Requests and History were four slices of one loan lifecycle, so they became one **Sharing** tab with two subtabs split by your role: each side stacks its own requests → active loans → past loans, and the library strip drops from eight tabs to five. The subtab pair reuses the pill control the loan history already used for exactly this axis, so its inner toggle is gone. Profile was untouched — it never had these tabs. "Past loans" reads the `resolved` slice (declined + returned) rather than `all`, which now paginates too: the in-flight rows have their own blocks above it and were otherwise listed twice on one screen
